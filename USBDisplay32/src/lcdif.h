...
//#define LCDBITINTERFACE
#ifdef LCDBITINTERFACE
	...
	*LCD_DATA = data;
    ...
    __attribute__((__always_inline__))
    #endif
    static inline void LCD_WriteCommand(unsigned short cmd)
    {
	*LCD_COMMAND = cmd;
    }
...
__attribute__((__always_inline__))
#endif
static inline void LCD_BltData( uint16_t data ) { LCD_WriteData(data); }

extern uint16_t g_BackgroundColour;
extern uint16_t g_ForegroundColour;
